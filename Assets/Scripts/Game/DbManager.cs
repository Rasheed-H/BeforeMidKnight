        ...
        using (var connection = new SQLiteConnection(dbPath))
        {
            ...
            using (var command = connection.CreateCommand())
            {
                ...
                command.CommandText = @"CREATE TABLE IF NOT EXISTS Stats (
                                    ...
                                    TotalSpiderKills INTEGER DEFAULT 0,
                                    TotalWizardKills INTEGER DEFAULT 0,
                                    TotalBossKills INTEGER DEFAULT 0)";
                command.ExecuteNonQuery();
                ...
                command.CommandText = @"INSERT OR IGNORE INTO Stats 
                                    (Id, HighestScore, HighestWeek, TotalDays, TotalCoinsDeposited, TotalCoinsLost, TotalDeaths, 
                                     TotalEscapes, TotalKills, TotalGoblinKills, TotalSkeletonKills, TotalSpiderKills, 
                                     TotalWizardKills, TotalBossKills) 
                                    VALUES 
                ...
                for (int i = 1; i <= 10; i++)
                {
                    command.CommandText = @"INSERT INTO Runs (Score, Week, Deaths, Escapes, TotalCoinsDeposited, Kills, Date) 
                                        VALUES (@score, @week, @deaths, @escapes, @totalCoinsDeposited, @kills, @date)";
                    command.Parameters.Clear();
                    command.Parameters.Add(new SQLiteParameter("@score", i * 100));
                    command.Parameters.Add(new SQLiteParameter("@week", i));
                    command.Parameters.Add(new SQLiteParameter("@deaths", i % 3));
                    command.Parameters.Add(new SQLiteParameter("@escapes", i % 2));
                    command.Parameters.Add(new SQLiteParameter("@totalCoinsDeposited", i * 50));
                    command.Parameters.Add(new SQLiteParameter("@kills", i * 10));
                    command.Parameters.Add(new SQLiteParameter("@date", System.DateTime.Now.AddDays(-i).ToString("yyyy-MM-dd HH:mm:ss")));
                    command.ExecuteNonQuery();
                }
                **/
            }
    ...
    public void UpdateStats(int highestScore, int highestWeek, int totalDays, int totalCoinsDeposited, int totalCoinsLost, int totalDeaths, int totalEscapes, int totalKills, int totalGoblinKills, int totalSkeletonKills, int totalSpiderKills, int totalWizardKills, int totalBossKills)
    {
        using (var connection = new SQLiteConnection(dbPath))
        {
            ...
            using (var command = connection.CreateCommand())
            {
                ...
                using (var reader = command.ExecuteReader())
                {
                    if (reader.Read())
                    {
                        ...
                        currentTotalSpiderKills = reader.GetInt32(reader.GetOrdinal("TotalSpiderKills"));
                        currentTotalWizardKills = reader.GetInt32(reader.GetOrdinal("TotalWizardKills"));
                        currentTotalBossKills = reader.GetInt32(reader.GetOrdinal("TotalBossKills"));
                    }
                }
            }
            ...
            using (var command = connection.CreateCommand())
            {
                ...
                command.Parameters.Add(new SQLiteParameter("@totalSpiderKills", totalSpiderKills));
                command.Parameters.Add(new SQLiteParameter("@totalWizardKills", totalWizardKills));
                command.Parameters.Add(new SQLiteParameter("@totalBossKills", totalBossKills));
                command.ExecuteNonQuery();
            }
        }
    }
